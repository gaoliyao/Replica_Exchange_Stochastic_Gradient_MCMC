...
def SGHMC(data, y, bayes_nn, eta=0.00002, L=5, alpha=0.01, V=1, v0=0.001, v1=0.1):
    ...
    for num, parameter in enumerate(parameters):
        ...
        if p.data_ptr() == bayes_nn.fc1.weight.data.data_ptr():
            p *= torch.sqrt((1 - bayes_nn.p_star) * v0 ** 2 + bayes_nn.p_star * v1 ** 2) / v1
        current_ps.append(p)
    ...
    for l in range(L):
        ...
        for i in range(len(current_ps)):
            ...
            if parameters[i].data_ptr() == bayes_nn.fc1.weight.data.data_ptr():
                reinitilize *= torch.sqrt((1 - bayes_nn.p_star) * v0 ** 2 + bayes_nn.p_star * v1 ** 2) / v1
    ...
    return current_paras, 1, temp_U.view(-1).data.tolist()[0]
