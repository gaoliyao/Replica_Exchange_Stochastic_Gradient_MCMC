...
class SynthDataset():
    ...
    def __init__(self, x_dim=100, num_clusters=10, seed=123456):
...
class Cifar100():
    ...
    def __init__(self, path, subsample=None):
        ...
        train_data = sio.loadmat(os.path.join(path, "train.mat"))
        test_data = sio.loadmat(os.path.join(path, "test.mat"))

        self.imgs = train_data["data"] / 255.
        print(self.imgs.shape)
        self.imgs = self.imgs * 2 - 1.
        self.imgs = self.imgs.reshape([-1, 3, 32, 32])
        self.imgs = np.transpose(self.imgs, [0, 2, 3, 1])

        self.test_imgs = test_data["data"] / 255.
        self.test_imgs = self.test_imgs * 2 - 1.
        self.test_imgs = self.test_imgs.reshape([-1, 3, 32, 32])
        self.test_imgs = np.transpose(self.test_imgs, [0, 2, 3, 1])

        self.labels = np.array([yy[0]-1 for yy in train_data["fine_labels"]])
        self.labels = one_hot_encoded(self.labels, 100)

        self.test_labels = np.array([yy[0]-1 for yy in test_data["fine_labels"]])
        self.test_labels = one_hot_encoded(self.test_labels, 100)
